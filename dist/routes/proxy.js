...
const CONVERSATION_CHAT_URL = process.env['CONVERSATION_CHAT_URL'] ?? 'http://conversation-chat:8082';
function buildOpenSessionBody(tenantId) {
    return {
        channel: 'web',
...
proxyRouter.post('/api/v1/sessions', async (req, res) => {
    const body = req.body;
...
proxyRouter.post('/api/v1/sessions/:sid/turns', async (req, res) => {
    ...
    try {
        const upstream = await fetch(`${CONVERSATION_CHAT_URL}/api/v1/sessions/${sid}/turns`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                Authorization: 'Bearer internal',
